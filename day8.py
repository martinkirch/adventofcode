...
complete_input = """cdafg dage fgdaec cdbfgae cge gcbdfa fdceb gfceab ge ecfgd | eg eg dfecag ge
...
microscopic_input = "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf"

INPUT = complete_input

samples = [[i.strip().split(" ") for i in line.split("|")] for line in INPUT.split("\n")]

count = 0
for sample in samples:
    for output in sample[1]:
        if len(output) in (2, 4, 3, 7):
            count += 1

print(f"how many times do digits 1, 4, 7, or 8 appear in output: {count}")
...
class Segment():
    ...
    def enlighten(self, candidateset):
        if self.candidates is None:
            self.candidates = candidateset
        else:
            self.candidates = self.candidates.intersect(candidateset)
    
    def darken(self, letter):
        self.candidates.discard(letter)
    
    @property
    def letter(self):
        return (self.candidates, self.negatives)
...
class Mapping():
    ...
    def push_1(self, candidateset):
        self.top_right.update(candidateset)
        self.bottom_right.update(candidateset)
        self.seen.update(candidateset)

    def push_4(self, candidateset):
        remaining = candidateset - self.seen
        self.top_left.update(remaining)
        self.middle.update(remaining)
        self.seen.update(candidateset)

    def push_7(self, candidateset):
        remaining = candidateset - self.seen
        self.top.update(remaining)
        self.seen.update(candidateset)
        self.seen.update(candidateset)

    def push_len_5(self, candidateset):
        remaining = candidateset - self.seen
        self.bottom.update(remaining)
        self.bottom_left.update(remaining)
        self.seen.update(candidateset)
    
    def push_len_6(self, candidateset):
        remaining = (Mapping.alphabet - candidateset).pop()
        if remaining in self.bottom_left:
            self.bottom_left = set([remaining])
            self.bottom.discard(remaining)
        if remaining in self.middle:
            self.middle = set([remaining])
            self.top_left.discard(remaining)
        if remaining in self.top_right:
            self.top_right = set([remaining])
            self.bottom_right.discard(remaining)

    def print(self):
        print(f"top: {self.top}")
        print(f"top_left: {self.top_left}")
        print(f"top_right: {self.top_right}")
        print(f"middle: {self.middle}")
        print(f"bottom: {self.bottom}")
        print(f"bottom_left: {self.bottom_left}")
        print(f"bottom_right: {self.bottom_right}")
...
for sample in samples:
    d = Mapping()
    translation = {}
    for output in sorted(sample[0], key=len):
        length = len(output)
        if length == 2:
            d.push_1(set(output))
            translation[_(output)] = 1
        elif length == 3:
            d.push_7(set(output))
            translation[_(output)] = 7
        elif length == 4:
            d.push_4(set(output))
            translation[_(output)] = 4
        elif length == 5:
            d.push_len_5(set(output))
        elif length == 6:
            d.push_len_6(set(output))
        elif length == 7:
            translation[_(output)] = 8
    translation[_(d.top | d.top_right | d.middle | d.bottom_left | d.bottom)] = 2
    translation[_(d.top | d.top_left | d.middle | d.bottom_right | d.bottom)] = 5
    translation[_(d.top | d.top_right | d.middle | d.bottom_right | d.bottom)] = 3
    translation[_(d.top | d.top_left | d.top_right | d.bottom_right | d.bottom_left | d.bottom)] = 0
    translation[_(d.top | d.top_left | d.middle | d.bottom_right | d.bottom_left | d.bottom)] = 6
    translation[_(d.top | d.top_left | d.top_right | d.middle | d.bottom_right | d.bottom)] = 9

    total += int(''.join(str(translation[_(output)]) for output in sample[1]))
